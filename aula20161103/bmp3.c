#include <stdio.h>  //224, 31,142       21, 14, 49
...
struct Pixel{
 ...
 int main(){
   ...
   unsigned int dword; // 4 byes
   imagem = fopen("pato.bmp", "rb");
    if(imagem == NULL)
    {
...
fseek(imagem, 2*sizeof(word), SEEK_CUR); //pulando os 2 word, pois eles não tem nada
...
for(j = 0; j < HPX; j++) {//(a cada linha da imagem)
    for (i= 0; i < WPX;i++){ //(a cada coluna da mesma linha)
        ...
        pixel2.B =  pixel.B;
         ...
         //while(pixel.R =  255 & pixel.G =  242 & pixel.B =  0);
      }
    if (AUX > 0){
        fseek(imagem, AUX, SEEK_CUR); // sem informacao util
        for(i=0; i < AUX; i++)
              fwrite(&byte, sizeof(byte), 1, imagem2); // sem informacao util
      }
   }
fclose(imagem2);
fclose(imagem);
